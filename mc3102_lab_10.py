...
Original file is located at
    https://colab.research.google.com/drive/1eMc2Ago-5ZmjgKpb_kQnayfnKFIQIRto
...
strikes = pl.read_csv("https://raw.githubusercontent.com/philhetzel/opan5510-class10/refs/heads/main/data/faa_strikes.txt",
                      separator="\t"
)

strikes = strikes.with_columns(
    pl.col("Collision Date and Time").str.to_datetime()
)
strikes
...
running_total_strikes = (
    ...
    .sort("Date")
    .with_columns(
        pl.col("daily_strikes").cum_sum().over("Date").alias("strikes_cumulative")
    )
...
damage_state = (
    ...
    .agg(damage = pl.col("Cost: Total $").sum())
...
greatest_strike_increase = (
    ...
    .group_by("date")
    .agg(daily_strikes = pl.col("Number of Strikes").sum())
    .sort("date")
    .with_columns(
        previous_day = pl.col("daily_strikes").shift(1)
    )
    .with_columns( # Separate with_columns to use the previous_day column
